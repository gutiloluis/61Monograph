...
To write the ME, all the possible states of the system and the transitions must be specified. Therefore, we need to know with detail which components are producing fluctuations and the ways in which they interact. However, in the Langevin equation the specific details are not necessary. For example, the global noise arises as a consequence of many phenomena that affects all the components in the cell. There could be many of them and it would be very difficult to model all of them explicitely. In the Langevin equation we include global noise only with the assumption that it is correlated among the different components, and this apparently simple assumption led to results that match the experiments \cite{pedraza05}.

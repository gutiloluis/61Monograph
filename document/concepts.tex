...
It can be proven that according to the previous properties, $P(k,\tau)$ is given by \todo{Prove?}
...
In the Gillespie algorithm, we take advantage of the properties of the Poisson process, namely its memorylessness and its merging properties. Assuming all the events are Poissonian, the interarrival times are exponentially distributed and therefore memoryless. The  \todo[inline]{More detail and rigor} \cite{bertsekas08}.
...
Where $K$ turns out to be the sum of the probabilities per unit time of all the events. \todo[inline]{More detail and rigor}

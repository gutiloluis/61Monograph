...
\begin{equation}
  \label{eq:noise2}
  \nu_2 = \frac{\nicefrac{k_p}{\gamma_r}}{1+\nicefrac{\gamma_p}{\gamma_r}}+1, \quad \quad \eta_2^2 = \frac{1}{\langle n_2\rangle}\left(\frac{\nicefrac{k_p}{\gamma_r}}{1+\nicefrac{\gamma_p}{\gamma_r}}+1\right).
\end{equation}

As can be noticed, the noise in the proteins is bigger than Poissonian. Define $b\coloneqq \nicefrac{k_p}{\gamma_r}$, the average number of proteins produced during a lifetime of a transcript, commonly called the \textit{burst size}. Taking the degradation rate for a protein to be much smaller than for RNA, we have $\nicefrac{\gamma_p}{\gamma_r} \ll 1$\footnote{Usually proteins are more stable than mRNAs, for \textit{E. coli}, $\gamma_r~ 1/(5$ min$)$ and $\gamma_p~ 1/(30$ min$)$}. Thus, the noise reduces to
